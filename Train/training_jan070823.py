...
def gravnet_model(
    ...
    x_in = Concatenate()([pre_selection["prime_coords"], pre_selection["features"]])
    ...
    for i in range(TOTAL_ITERATIONS):
        ...
        gndist = LLRegulariseGravNetSpace(scale=0.01, print_loss=True, project=True)(
            [gndist, pre_selection["coords"], gnnidx]
        )
...
PUBLISHPATH = "jkiesele@lxplus.cern.ch:~/Cernbox/www/files/temp/FCC/"
...
if not train.modelSet():
    ...
    train.setCustomOptimizer(
        tf.keras.optimizers.Nadam(  # clipnorm=1.,
            # epsilon=1e-2
        )
    )
    #
