...
class AmbiguousTruthToNoiseSpectator(LayerWithMetrics):
    ...
    def call(self, inputs):
        ...
        n_energy = SelectWithDefault(nidx, energy, 0.0)
        n_mask = SelectWithDefault(nidx, tf.ones_like(energy), 0.0)
        ...
        sw = tf.where(is_same, sw, 1.0)  # set ambiguous to spectator
        ...
        # metrics
        self.add_prompt_metric(same_f, self.name + "_non_amb_truth_fraction")
        if self.return_score:
...
class LLObjectValuePenalty(LLValuePenalty):
    def __init__(self, noise_scale=10.0, **kwargs):
        """
        Simple value penalty loss, tries to keep values around default using simple
    ...
    def loss(self, inputs):
        ...
        mask_k_m = SelectWithDefault(Msel, tf.ones_like(val), 0.0)  # K x V-obj x 1
        vloss = (self.default - val_k_m) ** 2
        vloss = tf.math.divide_no_nan(
            tf.reduce_sum(vloss, axis=1), tf.reduce_sum(mask_k_m, axis=1) + 1e-6
        )
        vloss = tf.reduce_mean(vloss)

        # now the noise
        is_noise = tf.cast(tidx < 0, dtype="float32")
        vloss += self.noise_scale * tf.math.divide_no_nan(
            tf.reduce_sum(is_noise * (self.default - val) ** 2),
...
class LLPushTracks(LossLayerBase):
    def loss(self, inputs):
        ...
        beta = beta[is_track[:, 0] > 0.0]
        t_idx = t_idx[is_track[:, 0] > 0.0]

        if x is not None:
            print(self.name, beta, t_idx, x[is_track[:, 0] > 0.0])

        tclass = tf.where(t_idx < 0, 0.0, tf.ones_like(beta))
...
class LLLocalEnergyConservation(LossLayerBase):
    def __init__(self, grad_rel_threshold=0.1, K=64, dist_scale=1.0, **kwargs):
        """
        Inputs:
    ...
    def max_arg_w(self, sel, w):
        sw_k_m = SelectWithDefault(sel, w, 0.0)
        srange_k_m = SelectWithDefault(
            sel, tf.range(tf.shape(w)[0])[..., tf.newaxis], -1
        )
    ...
    def scale_gradient(self, energy_kp, t_energy_kp, grad_rel_threshold):
        ...
        reldiff = (
            1.0 - energy_kp / t_energy_kp
        )  # [kp, 1], use to define gradient reduction for well predicted showers
        ...
        energy_kp = gradscaler * energy_kp + (1.0 - gradscaler) * tf.stop_gradient(
            energy_kp
        )
    ...
    def loss(self, inputs):
        ...
        if rs.shape[0] is None:
            return 0.0 * tf.reduce_sum(weight)
        ...
        t_idx, t_nper_k = normalise_index(
            t_idx, rs, True, return_n_per=True
        )  # now it's normalised and spanning row splits
        ...
        pred_sid, _ = normalise_index(
            pred_sid, rs, True, return_n_per=True
        )  # now it's normalised and spanning row splits
        ...
        energy_kp = tf.ragged.boolean_mask(
            pfc_energy, not_isnoise_kp[..., 0]
        )  # pfc_energy laready ragged
        energy_kp = self.scale_gradient(
            energy_kp, t_energy_kp, grad_rel_threshold
        )  # scale grad here
        ...
        # concat per rs
        t_energy_k = tf.RaggedTensor.from_row_splits(
            t_energy_k, t_rs, validate=False
        )  # we know this works
        ...
        energy_kp = rconcat([energy_kp, 0.0 * energy_kp], axis=-1)  # rec, truthzero
        t_energy_k = rconcat([0.0 * t_energy_k, t_energy_k], axis=-1)  # zerorec, truth
        ...
        distsq /= tf.reduce_mean(
            tf.reduce_max(distsq, axis=1)
        )  # max for each vertex, min over all

        m_energy = SelectWithDefault(idx, allenergies, 0.0)

        w_m_energy = tf.expand_dims(tf.exp(-6.0 * distsq / 2.0), axis=2) * m_energy
...
class LLDynOCThresholds(LossLayerBase):
    def __init__(self, purity_weight=0.5, object_energy_weight="modlog", **kwargs):
        """
        beta-weighted here
        """
...
class LLFullOCThresholds(LossLayerBase):
    def __init__(self, purity_weight=0.5, object_energy_weight="modlog", **kwargs):
        ...
        assert 0.0 < purity_weight < 1.0
        self.purity_weight = purity_weight

        assert (
            object_energy_weight == "log"
            or object_energy_weight == "lin"
            or object_energy_weight == "modlog"
            or object_energy_weight == "none"
        )
    ...
    def loss(self, inputs):
        ...
        ch_d_n /= tf.expand_dims(
            tf.gather_nd(d, c_idx), axis=2
        )  # [e, r, r, 1], normalised to distance scaler
        ...
        ch_pure = tf.where(
            tf.expand_dims(c_t_idx, axis=2) == ch_t_idx,
            tf.ones_like(ch_t_idx),
            tf.zeros_like(ch_t_idx),
        )
        ...
        c_mean_t_depe = tf.reduce_sum(
            ch_noitnoise * ch_energy * ch_depe, axis=2
        )  # [e, r, 1]
        c_mean_t_depe /= tf.reduce_sum(ch_noitnoise * ch_energy, axis=2) + 1e-3

        # expected collected energy
        c_depe = c_mean_t_depe  # [e, r, 1]
        c_eff = tf.reduce_sum(_thresh(ch_energy, ch_d_n, t_d), axis=2) / tf.reduce_mean(
            _thresh(tf.ones_like(ch_energy), ch_d_n, t_d), axis=2
        )
        ...
        ow_sum = (
            tf.reduce_sum(c_oweight * _thresh(c_ones, c_betas, t_b, True), axis=1)
            + 1e-6
        )

        # make sure noise does not get included

        purity = tf.reduce_sum(
            c_oweight * _thresh(c_purity, c_betas, t_b, True), axis=1
        )
        purity /= ow_sum
        purity = tf.reduce_mean(purity)
        efficiency = tf.reduce_mean(
            c_oweight * _thresh(c_eff, c_betas, t_b, True), axis=1
        )
        efficiency /= ow_sum
        ...
        loss = self.purity_weight * (1.0 - purity) + (
            1.0 - self.purity_weight
        ) * tf.abs(1.0 - efficiency)
...
class LLOCThresholds(LossLayerBase):
    def __init__(
        self, purity_target, lowest_t_b=0.01, lowest_t_d=0.02, highest_t_d=5.0, **kwargs
    ):
        ...
        assert highest_t_d > 0.0
        assert lowest_t_d > 0.0
        assert lowest_t_b >= 0.0
    ...
    def loss(self, inputs):
        ...
        beta_k_m = SelectWithDefault(Msel, beta, 0.0)  # K x V-obj x 1
        mask_k_m = SelectWithDefault(Msel, tf.ones_like(beta), 0.0)  # K x V-obj x 1
        x_k_m = SelectWithDefault(Msel, x, 0.0)  # K x V-obj x C
        d_k_m = SelectWithDefault(Msel, d, 0.0)
        tidx_k_m = SelectWithDefault(Msel, tidx, -2)  # K x V-obj x 1

        cluster_size = tf.reduce_sum(mask_k_m, axis=1)  # K x 1
        non_singular = tf.cast(cluster_size > 1.0, "float32")  # K x 1
        ...
        dsq_k_m = tf.reduce_sum(
            (tf.expand_dims(x_k, axis=1) - x_k_m) ** 2, axis=2, keepdims=True
        )  # K x V-obj x 1
        ...
        any_non_singular = tf.reduce_sum(non_singular) > 0.0
        ...
        p_w = tf.where(any_non_singular, p_w, 1.0)
        ...
        loss_b = 0.01 * tf.abs(t_b) + 0.01 * (1.0 - tf.abs(t_d) / self.highest_t_d)
        ...
        p_k_t = tf.reduce_sum(mask_k_m * same_k_m, axis=1) / tf.reduce_sum(
            mask_k_m, axis=1
        )

        # print('p_k_t',p_k_t.shape)

        p_k_t = tf.reduce_sum(non_singular * p_k_t) / (
            tf.reduce_sum(non_singular) + 1e-6
        )
        p_k_t = tf.where(any_non_singular, p_k_t, 1.0)
...
class LLEnergySums(LossLayerBase):
    ...
    def loss(self, inputs):
        ...
        tot = tf.constant(0.0, dtype="float32")
        if rs.shape[0] is None:
            return tot

        t_energy = tf.where(
            t_pid[:, id_str_to_idx["muon"] : id_str_to_idx["muon"] + 1] > 0,
            0.0,
            t_energy,
        )  # remove MIPS

        energy = tf.where(t_idx < 0, 0.0, energy)  # mask noise
        energy = tf.where(is_track > 0, 0.0, energy)  # mask tracks
...
class LLClusterCoordinates(LossLayerBase):
    ...
    def __init__(
        self, downsample: int = -1, ignore_noise=False, hinge_mode=False, **kwargs
    ):
        if "dynamic" in kwargs:
...
class LLLocalClusterCoordinates(LossLayerBase):
    ...
    def raw_loss(dist, nidxs, tidxs, specweight, print_loss, name, scaler=None):
        ...
        sel_spec = SelectWithDefault(nidxs, specweight, 1.0)[:, :, 0]
        active = tf.where(nidxs >= 0, tf.ones_like(dist), 0.0)
        notspecmask = 1.0  # (1. - 0.5*sel_spec)#only reduce spec #tf.where(sel_spec>0, 0., tf.ones_like(dist))

        probe_is_notnoise = tf.cast(tidxs >= 0, dtype="float32")[:, 0]  # V
        notnoisemask = tf.where(sel_tidxs < 0, 0.0, tf.ones_like(dist))
        notnoiseweight = notnoisemask + (1.0 - notnoisemask) * 0.01
        ...
        repmask = (1.0 - sameasprobe) * notspecmask * active

        attr = tf.math.log(tf.math.exp(1.0) * dist + 1.0) * attmask
        rep = (
            tf.exp(-dist) * repmask * notnoiseweight
        )  # 1./(dist+1.) * repmask #2.*tf.exp(-3.16*tf.sqrt(dist+1e-6)) * repmask  #1./(dist+0.1)
        ...
        lossval = tf.math.divide_no_nan(
            tf.reduce_sum(probe_is_notnoise * lossval), tf.reduce_sum(probe_is_notnoise)
        )
    ...
    def loss(self, inputs):
        ...
        return LLLocalClusterCoordinates.raw_loss(
            dist, nidxs, tidxs, specweight, self.print_loss, self.name, scaler=scaler
        )
...
class LLNotNoiseClassifier(LossLayerBase):
    ...
    def get_config(self):
        ...
        return dict(
            ...
            + list(
                {
                    "purity_metrics_threshold": self.purity_metrics_threshold,
    ...
    def raw_loss(score, tidx, specweight, name=""):
        ...
        classloss = (1.0 - specweight[:, 0]) * tf.keras.losses.binary_crossentropy(
            truth, score
        )
    ...
    def loss(self, inputs):
        ...
        accuracy = tf.reduce_sum(
            isnotnoise * tf.cast(score > 0.5, dtype="float32")
        ) / tf.reduce_sum(isnotnoise)
        ...
        if self.purity_metrics_threshold > 0:
            ...
            pur = tf.where(
                tf.reduce_sum(selnoise) == 0.0, 1.0, pur
            )  # define no selection as 1 purity
...
class LLNeighbourhoodClassifier(LossLayerBase):
    ...
    def raw_loss(score, nidx, tidxs, specweights):
        ...
        n_active = tf.where(
            nidx >= 0, tf.ones_like(nidx, dtype="float32"), 0.0
        )  # V x K
        truthscore = tf.math.divide_no_nan(
            tf.reduce_sum(n_good, axis=1, keepdims=True),
            tf.reduce_sum(n_active, axis=1, keepdims=True),
        )  # V x 1
        # cut at 90% same
        truthscore = tf.where(truthscore > 0.9, 1.0, truthscore * 0.0)  # V x 1
        ...
        noise_lossval = tf.math.divide_no_nan(
            tf.reduce_sum((1.0 - isnotnoise) * lossval), tf.reduce_sum(1.0 - isnotnoise)
        )

        lossval = (
            obj_lossval + 0.1 * noise_lossval
        )  # noise doesn't really matter so much
...
class LLEdgeClassifier(LossLayerBase):
    ...
    def get_config(self):
        ...
        return dict(
            ...
            + list(
                {
                    "downweight_spectators": self.downweight_spectators,
                    "fp_weight": self.fp_weight,
                    "hifp_penalty": self.hifp_penalty,
                    "lin_e_weight": self.lin_e_weight,
                }.items()
            )
        )
    ...
    def raw_loss(
        ...
        n_active = tf.where(nidx >= 0, tf.ones_like(nidx, dtype="float32"), 0.0)[
            :, 1:
        ]  # V x K-1
        specweight = tf.clip_by_value(specweight, 0.0, 1.0)
        energy = tf.clip_by_value(energy, 0.0, 100000.0)
        n_specw = SelectWithDefault(nidx, specweight, 1.0)[:, 1:, 0]  # V x K-1
        n_energy = (
            energy + SelectWithDefault(nidx, energy, 0.0)[:, 1:, 0]
        )  # V x K-1, energy sum of edge

        n_weight = (1.0 + tf.math.log(n_energy + 1.0)) + 1e-2
        weight = (1.0 + tf.math.log(energy + 1.0)) + 1e-2

        if lin_e_weight:
            weight = _calc_energy_weights(t_energy)

        if downweight_spectators:
            n_weight = (1.0 - n_specw) * n_weight + 1e-2
            weight = (1.0 - specweight) * weight + 1e-2
        ...
        n_mask_diff = (1.0 - n_sameasprobe[..., 0]) * n_active  # V x K-1
        # get class weights from here n_sameasprobe is already a mask
        all_sum = tf.reduce_sum(n_active)
        w_all_same = 1.0 - tf.reduce_sum(n_sameasprobe[..., 0] * n_active) / all_sum
        w_all_diff = 1.0 - w_all_same

        n_cl_weight = (
            2.0 * (1.0 - fp_weight) * n_sameasprobe[..., 0] * n_active * w_all_same
        )
        n_cl_weight += (
            2.0 * fp_weight * (1.0 - n_sameasprobe[..., 0]) * n_active * w_all_diff
        )

        # weight high scores more strongly
        score_w = 1.0 + hifp_penalty * tf.stop_gradient(score)[..., 0] ** 2
        score_w /= 1.0 + hifp_penalty
        ...
        lossval *= 2.0 * n_cl_weight  # class balance

        lossval = tf.math.divide_no_nan(
            tf.reduce_sum(lossval, axis=1), tf.reduce_sum(n_active, axis=1)
        )  # V
        lossval *= weight[:, 0]  # V
    ...
    def loss(self, inputs):
        ...
        if (
            nidx.shape[1] == score.shape[1]
        ):  # score should have one less, assume nidx does not contain 'self'
            nidx = tf.concat([tf.range(tf.shape(nidx)[0])[:, tf.newaxis], nidx], axis=1)

        lossval = LLEdgeClassifier.raw_loss(
            score,
            nidx,
            tidx,
            specweight,
...
class LLGoodNeighbourHood(LossLayerBase):
    def __init__(self, sampling=0.005, distscale=1.0, **kwargs):
    ...
    def get_config(self):
        ...
        return dict(
            list(base_config.items())
            + list({"sampling": self.sampling, "distscale": self.distscale}.items())
        )
    ...
    def loss(self, inputs):
        ...
        for i in range(len(rs) - 1):
            ...
            tsame = tf.cast(
                tf.expand_dims(stidx, axis=1) == tf.expand_dims(rstidx, axis=0),
                dtype="float32",
            )  # S x V x 1
            mdist = (
                tf.expand_dims(scoords, axis=1) - tf.expand_dims(rscoords, axis=0)
            ) ** 2
            mdist = tf.reduce_sum(mdist, axis=2) / sd**2  # S x V
            dist_weight = tf.exp(-mdist / 2.0)[:, :, tf.newaxis]  # S x V x 1

            dist_weight = tf.cast(mdist[:, :, tf.newaxis] < 1.0, "float32")

            sameprob = tf.math.divide_no_nan(
                tf.reduce_sum(tsame * dist_weight, axis=1),
                tf.reduce_sum(dist_weight, axis=1) + 1e-3,
            )  # S x 1
        ...
        self.add_prompt_metric(1.0 - cont, self.name + "_av_contamination")
        print(self.name, l, 1.0 - cont)
        return l


class LLKnnSimpleObjectCondensation(LossLayerBase):
    def __init__(self, K=196, purity_threshold=0.95, b_weight=0.5, **kwargs):

        assert 0.0 <= b_weight <= 1.0
    ...
    def same_truth_mask(self, t_idx, idx):
        ...
        notsameobj = tf.cast(
            tf.logical_or(tf.math.logical_not(sametidx), tf.math.logical_not(notnoise)),
            "float32",
        )
    ...
    def loss(self, inputs):
        ...
        # needed?
        ncoords = SelectWithDefault(idx, coords, 0.0)
        ...
        mask_k = SelectWithDefault(idx, 0.0 * beta + 1.0, 0.0)  # V x K x 1
        mask_k_noself = tf.concat([mask_k[:, 0:1] * 0.0, mask_k[:, 1:]], axis=1)
        ...
        purity = tf.math.divide_no_nan(
            tf.reduce_sum(purity, axis=1), tf.reduce_sum(pweight, axis=1) + 1e-3
        )  # K
        ispure = tf.cast(purity > self.purity_threshold, "float32")[:, tf.newaxis]

        mean_ispure = tf.reduce_mean(ispure)
        mean_ispure_no_noise = tf.reduce_sum(ispure) / tf.reduce_sum(
            tf.cast(t_idx >= 0, "float32")
        )
        ...
        self.add_prompt_metric(
            mean_ispure_no_noise, name=self.name + "_truth_nonoise_purity"
        )
        ...
        att_loss = tf.reduce_mean(
            tf.reduce_sum(att, axis=1) / tf.reduce_sum(mask_k_noself, axis=1) + 1e-3
        )
        rep_loss = tf.reduce_mean(
            tf.reduce_sum(rep, axis=1) / tf.reduce_sum(mask_k_noself, axis=1) + 1e-3
        )
        ...
        return self.b_weight * b_loss + (1.0 - self.b_weight) * (att_loss + rep_loss)


class LLKnnPushPullObjectCondensation(LossLayerBase):
    def __init__(self, q_min=0.8, s_b=1.0, K=64, mode="doubleexp", **kwargs):
    ...
    def rep_potential(self, dsq):
        return tf.exp(-dsq / 2.0)

    def att_potential(self, dsq):
        if self.mode == "doubleexp":
            return -tf.exp(-dsq / (2.0 * 0.5**2))
    ...
    def beta_loss(self, beta_ka):
        return tf.reduce_sum(beta_ka * 0.0)
    ...
    def loss(self, inputs):
        ...
        scaled_dsq = tf.math.divide_no_nan(
            distsq, d**2 + 1e-4
        )  # V x K, [:,0] is self

        beta_k = SelectWithDefault(idx, beta, 0.0)  # V x K x 1
        mask_k = SelectWithDefault(idx, 0.0 * beta + 1.0, 0.0)  # V x K x 1
        mask_k_noself = tf.concat([mask_k[:, 0:1] * 0.0, mask_k[:, 1:]], axis=1)
        ...
        att = (
            ...
            * mask_k_noself
            * attscaler
            * self.att_potential(scaled_dsq)[..., tf.newaxis]
        )
        rep = (
            notsameobj
            * mask_k_noself
            * repscaler
            * self.rep_potential(scaled_dsq)[..., tf.newaxis]
        )
        ...
        noise_loss = self.s_b * tf.math.divide_no_nan(
            tf.reduce_sum(beta * isnoise), tf.reduce_sum(isnoise) + 1e-3
        )
...
class LLBasicObjectCondensation(LossLayerBase):
    def __init__(
        self, q_min=0.8, s_b=1.0, use_average_cc_pos=0.5, implementation="std", **kwargs
    ):

        assert (
            implementation == "std"
            or implementation == "pushpull"
            or implementation == "precond"
        )
...
class LLFullObjectCondensation(LossLayerBase):
    ...
    def calc_position_loss(self, t_pos, pred_pos):
        ...
        ploss = (
            tf.sqrt(
                tf.reduce_sum((t_pos - pred_pos) ** 2, axis=-1, keepdims=True)
                / (10**2)
                + 1e-2
            ),
            10.0,
    ...
    def calc_timing_loss(self, t_time, pred_time, pred_time_unc, t_dep_energy=None):
        ...
        tloss = (
            tf.math.divide_no_nan(
                (t_time - pred_time) ** 2, (pred_time_unc**2 + 1e-1)
            )
            + pred_time_unc**2
        )
    ...
    def loss(self, inputs):
        ...
        energy_quantiles_loss *= self.energy_loss_weight / 2.0
        ...
        self.add_prompt_metric(
            tf.reduce_mean(pred_time_unc), self.name + "_time_pred_std"
        )
...
class PFTruthCondensateBuilder(object):
    def _hit_en_mean(
        ...
        if pfc_is_track is not None:
            ...
            x = tf.where(pfc_is_track > 0.0, tf.gather_nd(x_orig, pf_idx), x)
        return x

    def _truth_purity(self, pf_h_idx, pf_idx, t_idx, pfc_h_energy, pfc_hitensum):
        """
    ...
    def build_pf_truth(
        ...
        pfc_h_energy *= (
            1.0 - pfc_h_istrack
        )  # tf.where(pfc_h_istrack  > 0., 0., pfc_h_energy)#mask track energies
        ...
        out["pfc_t_encorr"] = self._hit_en_mean(
            encorr, pfc_h_energy, pfc_hitensum, pf_h_idx, pf_idx, pfc_istrack
        ).values

        out["pfc_t_energy"] = self._hit_en_mean(
            t_energy, pfc_h_energy, pfc_hitensum, pf_h_idx, pf_idx, pfc_istrack
        ).values

        out["pfc_t_rec_energy"] = self._hit_en_mean(
            t_rec_energy, pfc_h_energy, pfc_hitensum, pf_h_idx, pf_idx, pfc_istrack
        ).values

        out["pfc_t_pos"] = self._hit_en_mean(
            t_pos, pfc_h_energy, pfc_hitensum, pf_h_idx, pf_idx, pfc_istrack
        ).values
        ...
        out["pfc_t_fully_contained"] = tf.gather_nd(
            t_fully_contained, pf_idx
        ).values  # direct

        out["pfc_t_purity"] = self._truth_purity(
            pf_h_idx, pf_idx, t_idx, pfc_h_energy, pfc_hitensum
        ).values
...
class LLPFCondensates(LLFullObjectCondensation):
    ...
    def loss(self, inputs):
        ...
        pftruth = self.truthbuilder.build_pf_truth(
            pf_h_idx,
            pf_idx,
            pfc_istrack,
            rechit_energy,
        ...
        notnoise = 1.0 - pfc_isnoise
        ispure = tf.where(
            pftruth["pfc_t_purity"] < self.purity_threshold,
            0.0,
            tf.ones_like(pftruth["pfc_t_purity"]),
        )

        eweight = self.calc_energy_weights(
            pftruth["pfc_t_energy"], pftruth["pfc_t_id"], False
        )
        ...
        closs = (
            ...
            * notnoise
            * self.classification_loss_weight
            * self.calc_classification_loss(pftruth["pfc_t_id"], pfc_id)
        )
        ...
        tloss = (
            ...
            * self.calc_timing_loss(
                pftruth["pfc_t_time"], pfc_time, pfc_time_unc, pfc_t_rec_energy
            )
        )
        ...
        e_andunc_loss = (
            ...
            * self.calc_energy_correction_factor_loss(
                pftruth["pfc_t_energy"],
                pftruth["pfc_t_rec_energy"],
                pfc_mom_corr,
                pfc_energy_low_quantile,
                pfc_energy_high_quantile,
                return_concat=True,
        ...
        euncloss /= 2.0  # same as above

        self.add_prompt_metric(
            tf.reduce_mean(eweight * closs), self.name + "_class_loss"
        )
        self.add_prompt_metric(
            tf.reduce_mean(eweight * tloss), self.name + "_time_loss"
        )
        self.add_prompt_metric(tf.reduce_mean(eweight * ploss), self.name + "_pos_loss")
        self.add_prompt_metric(
            tf.reduce_mean(eweight * eloss), self.name + "_momentum_loss"
        )
        self.add_prompt_metric(
            tf.reduce_mean(eweight * euncloss), self.name + "_momentum_unc_loss"
        )
        ...
        resranges = [2.0, 10.0, 20, 50, 100.0, 200.0]
        for i in range(len(resranges) - 1):
            sel = tf.logical_and(
                pftruth["pfc_t_energy"] >= resranges[i],
                pftruth["pfc_t_energy"] < resranges[i + 1],
            )
            sel = sel[..., 0]
            ...
            var = tf.math.reduce_std(
                (s_pfc_energy - s_pfc_t_energy) / s_pfc_t_energy - offset
            )
...
class LLBasicObjectCondensation_nod(LossLayerBase):
    def __init__(
        self, q_min=0.8, s_b=1.0, use_average_cc_pos=0.5, implementation="std", **kwargs
    ):
        ...
        super(LLBasicObjectCondensation_nod, self).__init__(**kwargs)
        ...
        self.q_min = q_min
        self.s_b = s_b
        self.noise_q_min = None
        self.use_average_cc_pos = use_average_cc_pos
        self.dynamic_payload_scaling_onset = (-0.005,)

    def loss(self, inputs):
        assert len(inputs) == 4
        beta, coords, t_idx, rowsplits = inputs
        att, rep, noise, min_b, payload, exceed_beta = oc_loss(
            x=coords,
            beta=beta,
            truth_indices=t_idx,
