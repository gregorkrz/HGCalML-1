...
def create_outputs(
    ...
    pred_time_unc = Dense(1, activation="elu", name=name_prefix + "_time_unc")(
        ScalarMultiply(0.01)(x)
    )
    pred_time_unc = Add()(
    ...
    if not fix_distance_scale:
        pred_dist = ScalarMultiply(2.0)(
            Dense(1, activation="sigmoid", name=name_prefix + "_dist")(x)
        )
...
def re_integrate_to_full_hits(
    ...
    if "scatterids" in pre_selection.keys():
        ...
        pred_energy_corr = MultiBackScatterOrGather(default=1.0)(
            [pred_energy_corr, scatterids]
        )
        pred_energy_low_quantile = MultiBackScatterOrGather(default=1.0)(
            [pred_energy_low_quantile, scatterids]
        )
        pred_energy_high_quantile = MultiBackScatterOrGather(default=1.0)(
            [pred_energy_high_quantile, scatterids]
        )
        pred_pos = MultiBackScatterOrGather(default=0.0)([pred_pos, scatterids])
        pred_time = MultiBackScatterOrGather(default=10.0)([pred_time, scatterids])
        pred_id = MultiBackScatterOrGather(default=0.0)([pred_id, scatterids])
        pred_dist = MultiBackScatterOrGather(default=1.0)([pred_dist, scatterids])
...
from LossLayers import (
    ...
    AmbiguousTruthToNoiseSpectator,
    LLNotNoiseClassifier,
    LLBasicObjectCondensation,
    LLFillSpace,
    LLEdgeClassifier,
)
...
from LossLayers import (
    LLGoodNeighbourHood,
    LLOCThresholds,
    LLKnnPushPullObjectCondensation,
    LLKnnSimpleObjectCondensation,
)
...
def condition_input(orig_inputs, no_scaling=False):
    ...
    if not "coords" in orig_inputs.keys():
        ...
        orig_inputs[
            "features"
        ] = processed_features  #!the coordinates seem to be inputed twice

        # create starting point for cluster coords
        orig_inputs["prime_coords"] = XYZtoXYZPrime()(
...
def mini_gravnet_block(K, x, coords, rs, trainable, record_metrics, name):
    ...
    nidx, dist = KNN(
        K=K, record_metrics=record_metrics, name=name + "_knn", min_bins=20
    )(
        [knncoords, rs]
    ...
    x = DistanceWeightedMessagePassing(
        [32, 32], name=name + "dmp1", trainable=trainable, activation="elu"
    )(
        [x, nidx, dist]
    )  # hops are rather light
...
def mini_noise_block(sel, noise_threshold, trainable, record_metrics, name):
    ...
    isnotnoise = Where(outputval=1.0, condition="!=0")([sel["is_track"], isnotnoise])
    sel["isnotnoise_score"] = isnotnoise

    no_noise_sel, no_noise_rs, noise_backscatter = NoiseFilter(
        threshold=noise_threshold, record_metrics=record_metrics
    )([sel["isnotnoise_score"], sel["row_splits"]])

    for k in sel.keys():
        sel[k] = SelectFromIndices()(
            [no_noise_sel, sel[k]]
        )  # also  good check, will fail if dimensions don't match
...
def mini_pre_condensation_model(
    inputs,
    ...
    d = ScalarMultiply(2.0)(
        Dense(1, activation="sigmoid", name=name + "dense_d")(sel["x"])
    )

    ccoords = Dense(cluster_dims, name=name + "dense_ccoords", trainable=trainable)(
        sel["x"]
    )
    ccoords = ScalarMultiply(0.01)(ccoords)

    prime_coords = ElementScaling(name=name + "es2", trainable=trainable)(
        sel["prime_coords"]
    )
    prime_coords = expand_coords_if_needed(
        prime_coords, sel["x"], cluster_dims, name + "ccoords_exp", trainable=trainable
    )
    ...
    )([beta, ccoords, d, sel["t_spectator_weight"], sel["t_idx"], sel["row_splits"]])
    ...
    ch_idx, c_idx, _, revflat, asso_idx, dyn_t_d, dyn_t_b = RaggedCreateCondensatesIdxs(
        t_d=t_d,
        t_b=t_b,
        active=condensate,
        keepnoise=True,
        ...
        trainable=trainable,
        record_metrics=record_metrics,
        name=name + "_r_condensation",
    )([beta, ccoords, d, no_condensation_mask, sel["row_splits"]])

    out["cond_idx"] = OCReductionMetrics(
        name=name + "_metric", record_metrics=record_metrics
    )([asso_idx, sel["t_idx"]])
    ...
    if False:
        out["beta"] = LLOCThresholds(
            ...
            record_metrics=record_metrics,
        )([beta, out["cond_idx"], d, ccoords, sel["t_idx"], dyn_t_d, dyn_t_b])
    ...
    if debugplots_after > 0:
        ...
        out["dummy"] = PlotCoordinates(
            plot_every=debugplots_after,
            outdir=debug_outdir,
            name=name + "_ccoords_cleaning",
            publish=publishpath,
        )(
            [
                sel["coords"],
...
def pre_condensation_model(
    ...
    if debugplots_after > 0:
        coords = PlotCoordinates(
            plot_every=debugplots_after,
            outdir=debug_outdir,
            name=name + "_prime_coords",
            publish=publishpath,
        )([coords, energy, orig_inputs["t_idx"], orig_inputs["row_splits"]])
    ...
    x_e = Flatten()(x_e)
    x_e = Dense(
        32, activation="elu", name=name + "dense_flat_x_e_0", trainable=trainable
    )(x_e)
    x_e = Dense(
        32, activation="elu", name=name + "dense_flat_x_e_1", trainable=trainable
    )(x_e)

    x_e = ScaledGooeyBatchNorm(
        record_metrics=record_metrics,
        fluidity_decay=1e-2,  # fast decay
        trainable=trainable,
        name=name + "_gooey_2",
    ...
    x = Dense(
        64, activation="elu", name=name + "dense_bef_lastmp", trainable=trainable
    )(x)

    x = DistanceWeightedMessagePassing(
        [32, 32], name=name + "dmp2", trainable=trainable, activation="elu"
    )([x, nidx, dist])
    ...
    if noise_threshold > 0:
        ...
        isnotnoise = Where(outputval=1.0, condition="!=0")([is_track, isnotnoise])
    ...
    selcoordsscaled = ElementScaling(name=name + "es2", trainable=trainable)(
        sel["coords"]
    )
    exporigcoords = expand_coords_if_needed(
        selcoordsscaled,
        sel["x"],
        cluster_dims,
        name + "ccoords_exp",
    ...
    if condensation_mode == "pushpull":
        beta = LLKnnPushPullObjectCondensation(
            q_min=q_min,
            mode="dippedsq",
            active=trainable,
            record_metrics=record_metrics,
        ...
        beta = LLKnnSimpleObjectCondensation(
            active=trainable,
...
def pre_selection_model(
    ...
    if pf_mode:
        coord_mult = Dense(
            32, activation="elu", name=name + "dcoord1", trainable=trainable
        )(coords)
        coord_mult = Dense(
            3, activation="sigmoid", name=name + "dcoord2", trainable=trainable
        )(coord_mult)
    ...
    x = DistanceWeightedMessagePassing(
        [32, 32], name=name + "dmp1", activation=activation, trainable=trainable
    )(
        [x, nidx, dist]
    )  # hops are rather light
    ...
    if not pf_mode:
        dist, nidx = SortAndSelectNeighbours(K=-1, descending=True)(
            [dist, nidx, energy]
        )
    else:
    ...
    if pf_mode:
        x_e = Dense(
            edge_complexity[1] * K,
            activation=activation,
            name=name + "dense_flat_x_e_2",
            trainable=trainable,
        )(x_e)

    x_e = Reshape((K, edge_complexity[1]))(x_e)
    x_e = Dense(1, activation="sigmoid", name=name + "_ed3", trainable=trainable)(
        x_e
    )  # edge classifier
    ...
    x_e = LLEdgeClassifier(
        name=name + "_LLEdgeClassifier",
        active=trainable,
        record_metrics=record_metrics,
        scale=5.0,  # high scale
    ...
    if debugplots_after > 0:
        x_e = PlotEdgeDiscriminator(
            plot_every=debugplots_after,
            publish=publish,
            outdir=debug_outdir,
            name=name + "_edges",
        )([x_e, nidx, cluster_tidx, orig_inputs["t_energy"]])

    # skip the rest
    if pre_train_mode:
        return {"x_e": x_e}

    sel_nidx = EdgeSelector(threshold=reduction_threshold)([x_e, nidx])
    ...
    g_sel_nidx, g_sel, group_backgather, g_sel_rs = NeighbourGroups(
        threshold=1e-3, return_backscatter=False, record_metrics=False
    )([hierarchy, sel_nidx, rs])

    g_sel = MLReductionMetrics(
        name=name + "_reduction_0", record_metrics=record_metrics
    ...
    mean_orig_feat = AccumulateNeighbours("mean")(
        [orig_inputs["orig_features"], g_sel_nidx, energy]
    )
    ...
    if debugplots_after > 0:
        out["coords"] = PlotCoordinates(
            plot_every=debugplots_after, outdir=debug_outdir, name=name + "_after_red"
        )([out["coords"], out["rechit_energy"], out["t_idx"], g_sel_rs])
    ...
    if filter_noise:
        ...
        notnoisetruth = Where(outputval=1, condition=">0")(
            [out["t_spectator_weight"], out["t_idx"]]
        )
        # tracks are never noise here
        notnoisetruth = Where(outputval=1, condition=">0")(
            [out["is_track"], notnoisetruth]
        )

        isnotnoise = LLNotNoiseClassifier(
            active=trainable, record_metrics=record_metrics, scale=1.0
        )([isnotnoise, notnoisetruth])

        # tracks are never noise here**2
        isnotnoise = Where(outputval=1.0, condition=">0")([out["is_track"], isnotnoise])

        if debugplots_after > 0:
            isnotnoise = PlotNoiseDiscriminator(
                plot_every=debugplots_after,
                outdir=debug_outdir,
                name=name + "_noise_score",
            )([isnotnoise, notnoisetruth])
    ...
    out["row_splits"] = SimpleReductionMetrics(
        name=name + "full_reduction", record_metrics=record_metrics
    )([out["row_splits"], orig_inputs["row_splits"]])

    beta = Dense(
        1, activation="sigmoid", trainable=trainable, name=name + "_proto_beta"
    )(out["features"])
    d = ScalarMultiply(2.0)(
        Dense(1, activation="sigmoid", trainable=trainable, name=name + "_proto_d")(
            out["features"]
        )
    )
    if trainable:
...
from RaggedLayers import (
    RaggedCreateCondensatesIdxs,
    RaggedSelectFromIndices,
    RaggedMixHitAndCondInfo,
)
from RaggedLayers import (
    RaggedCollapseHitInfo,
    RaggedDense,
    RaggedToFlatRS,
    FlatRSToRagged,
    ForceExec,
...
def noise_filter_block(
    ...
    # tracks are never noise here**2
    isnotnoise = Where(outputval=1.0, condition=">0")(
...
def pre_selection_model2(
    ...
    x_track = Dense(
        32, activation="elu", name=name + "emb_xtrack", trainable=trainable
    )(x)
    x_hit = Dense(32, activation="elu", name=name + "emb_xhit", trainable=trainable)(x)
    x = MixWhere()(
        [orig_inputs["is_track"], x_track, x_hit]
    )  # Concatenate()([coords,x,good_track_feat])

    good_track_feat = ScalarMultiply(1.0 / 20.0)(
        SelectFeatures(9, 10)(orig_inputs["features"])
    )
    good_track_feat = Where(0.0, "==0")([orig_inputs["is_track"], good_track_feat])
    ...
    coords = ElementScaling(name=name + "es1", trainable=trainable)(coords)
    nidx, dist = KNN(
        K=K, record_metrics=record_metrics, name=name + "_np_knn", min_bins=20
    )(
        [coords, orig_inputs["row_splits"]]
    )  # hard code it here, this is optimised given our datasets
    x = DistanceWeightedMessagePassing(
        [32], name=name + "np_dmp1", activation=activation, trainable=trainable
    )(
        [x, nidx, dist]
    )  # hops are rather light
    x = Dense(
        32, activation=activation, name=name + "dense_np_1a", trainable=trainable
    )(x)
    x = Dense(
        32, activation=activation, name=name + "dense_np_1b", trainable=trainable
    )(x)
    ...
    x_track = Dense(32, activation="elu", name=name + "d_xtrack", trainable=trainable)(
        x
    )
    x_hit = Dense(32, activation="elu", name=name + "d_xhit", trainable=trainable)(x)
    x_c = MixWhere()(
        [is_track, x_track, x_hit]
    )  # Concatenate()([coords,x,good_track_feat])

    # this is important
    x_c = Dense(32, activation="elu", name=name + "d_coord1", trainable=trainable)(x_c)
    coords_m = Dense(
        3, activation="tanh", name=name + "d_coord2mult", trainable=trainable
    )(x_c)
    ...
    if debugplots_after > 0:
        coords = PlotCoordinates(
            plot_every=debugplots_after, outdir=debug_outdir, name=name + "_pre_coords"
        )([coords, energy, is_track, nidx, t_idx, rs])
    ...
    x_gn = DistanceWeightedMessagePassing(
        [32, 32], name=name + "dmp1", activation=activation, trainable=trainable
    )(
        [x, nidx, dist]
    )  # hops are rather light
    ...
    if True:
        for i in range(n_itheads):
            ...
            dist = Dense(
                K + 1,
                activation="relu",
                name=name + "dense2_att" + str(i),
                trainable=trainable,
            )(dist)
            x_gn = DistanceWeightedMessagePassing(
                [32],
                name=name + "dmp_att" + str(i),
                activation=activation,
                trainable=trainable,
            )(
                [x_gn, nidx, dist]
        ...
        x_gn = AttentionMP(4 * [16], K + 1, 4)([x_gn, nidx])  # position encoding
    ...
    x = Dense(32, activation=activation, name=name + "dense1a", trainable=trainable)(
        x_gn
    )
    x = Dense(32, activation=activation, name=name + "dense1b", trainable=trainable)(x)
    ...
    if flat_edges:
        x_e = Dense(
            24, activation=activation, name=name + "dense_x_e", trainable=trainable
        )(x)
        x_e = SelectFromIndicesWithPad()([nidx, x_e])  # this will be big
        x_e = Flatten()(x_e)
        x_e = Dense(
            16 * K,
            activation=activation,
            name=name + "dense_flat_x_e1",
            trainable=trainable,
        )(x_e)
        x_e = Dense(
            16 * K,
            activation=activation,
            name=name + "dense_flat_x_e2",
            trainable=trainable,
        )(x_e)
        ...
        x_e = Dense(
            32, activation=activation, name=name + "dense_x_e", trainable=trainable
        )(
            x_e
        )  # 96
        x_e = EdgeCreator()([nidx, x_e])
        x_e = Dense(
            32, activation=activation, name=name + "dense_x_e1", trainable=trainable
        )(
            x_e
        )  # 64
        x_e = Dense(
            16, activation=activation, name=name + "dense_x_e2", trainable=trainable
        )(
            x_e
        )  # 32
        x_e = Dense(
            16, activation=activation, name=name + "dense_x_e3", trainable=trainable
        )(
            x_e
        )  # 32
    ...
    if debugplots_after > 0:
        x_e = PlotEdgeDiscriminator(
            ...
            outdir=debug_outdir,
            name=name + "_edges",
        )([x_e, nidx, cluster_tidx, no_noise["t_energy"]])
    ...
    sel_t_spec_w, sel_t_idx, unamb_score = AmbiguousTruthToNoiseSpectator(
        return_score=True, record_metrics=record_metrics
    )([sel_nidx, t_spec_w, t_idx, energy])
    ...
    corr_rechit_energy = AccumulateNeighbours("sum")(
        [no_noise["corr_rechit_energy"], g_sel_nidx]
    )
    ...
    prime_coords_o = AccumulateNeighbours("mean")(
        [no_noise["prime_coords"], g_sel_nidx, energy]
    )
    prime_coords_o = SelectFromIndices()([g_sel, prime_coords_o])

    # pass original features
    mean_orig_feat = AccumulateNeighbours("mean")(
        [no_noise["orig_features"], g_sel_nidx, energy]
    )
    mean_orig_feat = SelectFromIndices()([g_sel, mean_orig_feat])
...
def tiny_intermediate_condensation(
    ...
    d = ScalarMultiply(2.0)(Dense(1, activation="sigmoid", name=name + "dense_d")(x))
    ccoords = Dense(cluster_dims, name=name + "dense_ccoords", trainable=trainable)(x)

    beta = LLBasicObjectCondensation(
        q_min=0.2,  # high qmin here
        implementation="std",
        record_batch_time=record_metrics,
        scale=0.2,
        active=trainable,
    ...
    ch_idx, c_idx, rev_r, revflat, asso_idx = RaggedCreateCondensatesIdxs(
        t_d=0.25, t_b=0.1, return_thresholds=False
    )([beta, ccoords, d, rs])
...
def intermediate_condensation(
    ...
    # low thresholds
    ch_idx, c_idx, _, revflat, asso_idx = RaggedCreateCondensatesIdxs(
        t_d=0.25, t_b=0.1, collapse_noise=False, return_thresholds=False
    )([beta, ccoords, d, rs])
    # t_d, t_b
    # if False:
    #    beta = LLOCThresholds(
    ...
    if True:
        xf, _, gnnidx, gndist = RaggedGravNet(
            n_neighbours=32,
            n_dimensions=3,
            n_filters=64,
            n_propagate=64,
...
from GravNetLayersRagged import (
    SelfAttention,
    ScaledGooeyBatchNorm2,
    EdgeConvStatic,
    SplitOffTracks,
    ConcatRaggedTensors,
    ConditionalScaledGooeyBatchNorm,
)
...
def pre_graph_condensation(
    ...
    x = ScaledGooeyBatchNorm2(
        name=name + "_cond_batchnorm_a", record_metrics=record_metrics
    )([x, is_track])
    ...
    if trainable:
        for k in orig_inputs.keys():
            if (not "t_" == k[0:2]) and (not "row_splits" in k):
                orig_inputs[k] = CheckNaN(name=name + "_pre_check_postnorm_" + k)(
                    orig_inputs[k]
                )
    ...
    if first_call:
        ...
        x_hit = Dense(
            32, activation="elu", name=name + "emb_xhit", trainable=trainable
        )(x)
    ...
    coords = expand_coords_if_needed(
        coords, x, 4, name=name + "_exp_coords", trainable=trainable
    )
    ...
    x = ScaledGooeyBatchNorm2(
        name=name + "_batchnorm1", trainable=trainable, record_metrics=record_metrics
    )(x)

    x = Concatenate()([SelfAttention(name=name + "_selfatt2")(x), x])
    x = Dense(
        32, activation=activation, name=name + "dense_np_0b", trainable=trainable
    )(x)
    ...
    x = ScaledGooeyBatchNorm2(
        name=name + "_batchnorm2", trainable=trainable, record_metrics=record_metrics
    )(x)

    x = Concatenate()([SelfAttention(name=name + "_selfatt3")(x), x])
    x = Dense(
        32, activation=activation, name=name + "dense_np_0c", trainable=trainable
    )(x)
    ...
    x = RaggedGlobalExchange(skip_min=True)(
        [x, rs]
    )  # not a lot of information in minimum due to elu activation
    x = ScaledGooeyBatchNorm2(
        name=name + "_batchnorm3", trainable=trainable, record_metrics=record_metrics
    )(x)
    ...
    x = ScaledGooeyBatchNorm2(
        name=name + "_batchnorm4", trainable=trainable, record_metrics=record_metrics
    )(x)

    if dynamic_spectators:
        tmp_spec = Dense(1, activation="sigmoid", name=name + "_dyn_specw")(x)
        orig_inputs["t_spectator_weight"] = LLValuePenalty(
    ...
    coords = LLClusterCoordinates(
        ...
        print_batch_time=True,
        hinge_mode=True,
    )([coords, orig_inputs["t_idx"], orig_inputs["t_spectator_weight"], score, rs])

    if debugplots_after > 0:
        coords = PlotCoordinates(
            plot_every=debugplots_after,
            outdir=debug_outdir,
            name=name + "_cond_coords",
            publish=publish,
        )(
            [
                coords,
    ...
    trans_a = CreateGraphCondensation(score_threshold=0.5, K=5)(
    ...
    x_e = LLGraphCondensationEdges(active=trainable, record_metrics=record_metrics)(
    ...
    out["prime_coords"] = PushUp(add_self=True)(
        orig_inputs["prime_coords"], trans_a, weight=energy
    )
    ...
    if debugplots_after > 0:
        ...
        coords = PlotGraphCondensation(
            plot_every=debugplots_after, outdir=debug_outdir, publish=publish
        )(
            [
                coords,
                Where(0.1)([is_track, energy]),  # make tracks small in plot
                trans_a["weights_down"],
                trans_a["nidx_down"],
                rs,
            ]
        )
        ...
        orig_inputs["t_energy"] = PlotGraphCondensationEfficiency(
            ...
            publish=publish,
        )(orig_inputs["t_energy"], orig_inputs["t_idx"], trans_a)

        out["prime_coords"] = PlotCoordinates(
            plot_every=debugplots_after,
            outdir=debug_outdir,
            name=name + "_after_coords",
            publish=publish,
        )(
            [
                out["prime_coords"],
    ...
    out["down_features"] = Concatenate()(
        [x, x_skip, x_in]
    )  # this is for further "bouncing"
...
def mini_pre_graph_condensation(
    ...
    x = ConditionalScaledGooeyBatchNorm(
        name=name + "_cond_batchnorm", record_metrics=record_metrics
    )([x, is_track])
    ...
    if first_call:
        ...
        # pre-processing
        x_track = Dense(
            16, activation="elu", name=name + "emb_xtrack", trainable=trainable
        )(x)
        # x_track = Dense(32, activation='elu', name=name+'emb_xtrack1',trainable=trainable)(x_track)
        x_hit = Dense(
            16, activation="elu", name=name + "emb_xhit", trainable=trainable
        )(x)
        # x_hit = Dense(32, activation='elu', name=name+'emb_xhit1',trainable=trainable)(x_hit)
        x = MixWhere()(
            [is_track, x_track, x_hit]
    ...
    trans_a = CreateGraphCondensation(score_threshold=score_threshold, K=5)(
        score, coords, rs, always_promote=is_track
    )
    ...
    if produce_output:
        ...
        out["coords"] = PushUp(add_self=True)(
            orig_inputs["coords"], trans_a, weight=energy
        )
        out["rechit_energy"] = PushUp(mode="sum", add_self=True)(energy, trans_a)
...
def intermediate_graph_cleaning(
    ...
    trans_a = CreateGraphCondensation(score_threshold=score_threshold, K=K)(
        score, coords, rs, always_promote=is_track
    )
...
def intermediate_graph_condensation(
    ...
    coords = Dense(
        n_cond_coords, name=name + "_cond_coords", use_bias=False, trainable=trainable
    )(x)
    ...
    score = LLGraphCondensationScore(
        ...
        active=trainable,
        penalty_fraction=0.5,
        low_energy_cut=1.0,  # allow everything below 1 GeV to be removed
    )([score, coords, orig_inputs["t_idx"], orig_inputs["t_energy"], rs])
...
def tiny_pc_pool(
    ...
    x = ScaledGooeyBatchNorm2(name=name + "_cond_batchnorm_a", trainable=trainable)(
        [x, is_track]
    )

    x = ScaledGooeyBatchNorm2(
        name=name + "_cond_batchnorm_b", trainable=trainable, invert_condition=True
    )([x, is_track])
    ...
    # simple gravnet
    nidx, dist = KNN(
        K=K_nn, record_metrics=record_metrics, name=name + "_np_knn", min_bins=20
    )(
        [
            coords,  # stop gradient here as it's given explicitly below
            orig_inputs["row_splits"],
        ]
    )  # hard code it here, this is optimised given our datasets
    ...
    score = Dense(
        1, activation="sigmoid", name=name + "_pcp_score", trainable=trainable
    )(x)
    ...
    score = LLGraphCondensationScore(
        ...
        record_metrics=record_metrics,
        K=K_loss,
        penalty_fraction=1.0 - reduction_target,
        active=trainable,
    ...
    trans_a = CreateGraphCondensation(
        ...
        score,
        coords,
        rs,  # nidx,dist_orig, #do not use nidx here yet.
    ...
    if new_format:
        ...
        x_e = Dense(
            32, name=name + "_graphn_edges_d1", trainable=trainable, activation="elu"
        )(x_e)
        x_e = Dense(
            K_gp + 1,
            name=name + "_graphn_edges_d2",
            trainable=trainable,
            activation="softmax",
        )(
            x_e
    ...
    out["features"] = Concatenate()(
        [out["prime_coords"], out["up_features"], x_of, x_of2]
    )
